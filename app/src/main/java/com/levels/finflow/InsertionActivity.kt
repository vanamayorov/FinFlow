...
class InsertionActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnSaveData.setOnClickListener {
            if (!isSubmitted){
                ...
                Snackbar.make(findViewById(android.R.id.content), "Ви зберегли дані про транзакцію", Snackbar.LENGTH_LONG).show()
            }
    ...
    private fun saveTransactionData() {
        ...
        if(amountEt.isEmpty()){
            ...
            etTitle.error = "Введіть назву"
        }else if(category.isEmpty()){
            etCategory.error = "Оберіть категорію"
        }else{
            ...
            dbRef.child(transactionID).setValue(transaction)
                .addOnCompleteListener {
                    Toast.makeText(this, "Дані успішно добавлено", Toast.LENGTH_LONG).show()
                    finish()
